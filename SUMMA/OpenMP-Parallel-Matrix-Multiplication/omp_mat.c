...
******/
// Compilation MacBook: gcc -openmp -g -Wall -std=c99 omp_mat.c -o omp_matMult -lm
// Compilation Cluster: gcc -fopenmp -g -Wall -std=c99 omp_mat.c -o omp_mat -lm
...
/*** Standard Parameter:
	...
	- THRESHOLD = 0.001
	- iteration = 10
	- dimension = 128 to 4096 (iterated in for()-loop !!)
	- numThreads = 8 to 128 (iterated in for()-loop !!)
***/
...
double parallelMultiply_omp_opti(double* matrixA, double* matrixB, double* matrixC, int dimension, int numThreads);
...
void parallelMultiplyTest_omp_opti(int dimension, int iterations, int numThreads);
...
int main(int argc, char* argv[]) {
	...
	fp = fopen("parallelMultiplyTest_omp_opti.txt", "w+");
	fclose(fp);
	...
	for(dimension=MIN_DIM; dimension<=MAX_DIM; dimension=dimension*2) {
		for(numThreads=MIN_THREAD; numThreads<=MAX_THREAD; numThreads=numThreads*2) {
			parallelMultiplyTest_omp_opti(dimension, iterations, numThreads);
		}
	}
...
/*********** parallel Matrix Multiplication with OpenMP and helper scalar for loop invariant optimization **********/
double parallelMultiply_omp_opti(double* matrixA, double* matrixB, double* matrixC, int dimension, int numThreads) {
// Parallel multiply given input matrices using optimal methods and return resultant matrix
	int i, j, k; // Iterators
	double C_tmp; // helper scalar to not recalculate C(i,j) again and again in inner loops
	double elapsed; // time difference
	// take the time
	/*struct timeval t0, t1;
	gettimeofday(&t0, 0);*/
	...
	t0 = omp_get_wtime();
	/* Head */
	#pragma omp parallel private(i, j, k, C_tmp) num_threads(numThreads)
	{
		#pragma omp for schedule(static)
		for(i=0; i<dimension; i++) {
			for(j=0; j<dimension; j++) {
				C_tmp = 0.0; // helper C must be set to zero after every 2 outer loops
				for(k=0; k<dimension; k++) {
					C_tmp += matrixA[i*dimension + k] * matrixB[k*dimension + j]; // just a fast scalar operation
				}
				matrixC[i*dimension + j] = C_tmp; // give value of helper C to C(i,j)
			}
		}
	}
	/* Tail */
...
void parallelMultiplyTest_omp_opti(int dimension, int iterations, int numThreads) {
	FILE* fp;
	fp = fopen("parallelMultiplyTest_omp_opti.txt", "a+");

	// Console write
	printf("----------------------------------\n");
	printf("Test : Parallel Multiply OMP optimized\n");
	printf("----------------------------------\n");
	printf("Dimension: %d\tNo. of Threads: %d\n", dimension, numThreads);
	printf("..................................\n");
	...
	// File write
	fprintf(fp, "----------------------------------\n");
	fprintf(fp, "Test : Parallel Multiply OMP optimized\n");
	fprintf(fp, "----------------------------------\n");
	fprintf(fp, "Dimension: %d\tNo. of Threads: %d\n", dimension, numThreads);
	fprintf(fp, "..................................\n");
	...
	for(i=0; i<iterations; i++) {
		matrixResult = zeroMatrix(dimension);
		opmLatency[i] = parallelMultiply_omp_opti(matrixA, matrixB, matrixResult, dimension, numThreads);
		// check the result of the simple OpenMP parallel Multiplication
