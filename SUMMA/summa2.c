/*
* originally from: https://github.com/kanji-ji/SUMMA/blob/master/mat-mat.c
* SUMMA / mat-mac.c
* has SHELL Script included...
############################################
#!/bin/bash
...
mpiexec.hydra -n ${PJM_MPI_PROC} ./mat_mat
source /usr/local/bin/mpi_core_setting.sh
mpiexec.hydra -n ${PJM_MPI_PROC} ./mat_mat
export I_MPI_HBW_POLICY=hbw_preferred
mpiexec.hydra -n ${PJM_MPI_PROC} ./mat_mat
############################################
* has also the input data as a text-document included
*/
...
// file format is complicated, but as .txt is best i think
void read_matrix(double* A, char* filename, MPI_Comm comm_cart) {
    int coords[2];
    MPI_Cart_coords(comm_cart, myid, 2, coords);
    FILE *fp;
...
void plus_matrix(double* A, double* B, int n)
    int i, j;
...
void SUMMA(MPI_Comm comm_cart, double* C, double* A, double* B, int n) {
